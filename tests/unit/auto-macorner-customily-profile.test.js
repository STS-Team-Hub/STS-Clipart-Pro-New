...
function mk(tag, cls=''){ return { tagName:tag.toUpperCase(), className:cls, attrs:{}, textContent:'', children:[], parentElement:null,
  ...
  closest(sel){ let n=this; while(n){ if(sel==='.customily_option' && String(n.className||'').split(/\s+/).includes('customily_option')) return n; n=n.parentElement; } return null; },
  ...
  querySelectorAll(s){ const sels=s.split(',').map(x=>x.trim()); const out=[]; const has=(n,c)=>String(n.className||'').split(/\s+/).includes(c);
    const m=(n,sel)=> sel==='.customily_option'?has(n,'customily_option'):sel==='.title-customize'?has(n,'title-customize'):sel==='.option_name'?has(n,'option_name'):sel==='label[role="tab"]'?(n.tagName==='LABEL'&&n.attrs.role==='tab'):sel==='[role="tab"]'?(n.attrs.role==='tab'):sel==='label[aria-controls]'?(n.tagName==='LABEL'&&!!n.attrs['aria-controls']):sel==='input[type="text"]'?(n.tagName==='INPUT'&&n.attrs.type==='text'):sel==='input[type="email"]'?false:sel==='input[type="tel"]'?false:sel==='input[type="number"]'?false:sel==='input:not([type])'?false:false;
...
const g1=root.appendChild(mk('div','customily_option')); g1.attrs['sort-id']='1'; g1.attrs['opt-id']='g1'; const lab1=g1.appendChild(mk('label')); lab1.attrs.role='tab'; lab1.attrs['aria-controls']='panel1'; const o1=lab1.appendChild(mk('div','option_name')); o1.textContent='Option 1 of 4 Choose Style (0|20) * Required';
const g2=root.appendChild(mk('div','customily_option')); g2.attrs['sort-id']='2'; g2.attrs['opt-id']='g2'; const lab2=g2.appendChild(mk('label')); lab2.attrs.role='tab'; lab2.attrs['aria-controls']='panel2'; const o2=lab2.appendChild(mk('div','option_name')); o2.textContent='Enter Name'; const inp=g2.appendChild(mk('input')); inp.attrs.type='text'; inp.attrs.name='properties[Kid Name]'; inp.attrs.placeholder='Enter name'; inp.attrs.maxlength='20';
...
const manualCandidates = p.getManualDrivenAutoTitleCandidates({ document: doc, location: { hostname: 'www.macorner.co' } });
assert.deepEqual(manualCandidates.map(c => c.label), ['Choose Style', 'Enter Name']);
assert.equal(manualCandidates[0].titleEl, o1);
assert.equal(manualCandidates[0].groupEl, g1);
assert.equal(manualCandidates[0].expandEl, lab1);
assert.equal(p.getAutoExpandTarget(o2), lab2);
(async()=>{
